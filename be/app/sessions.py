...
def _carry_routing_fields(payload: Any, metadata: Any) -> Any:
    """Copy the routing fields off the metadata half before it is dropped.

    ``run_id`` and ``langgraph_node`` live only in the metadata, and each SDK
    needs one of them to attribute a continuation frame to the message it
    belongs to. ``MessagesData`` payloads are ``Extensible``, so adding ``id``
    is an allowed extension rather than a schema violation.
    """
    if not isinstance(payload, dict) or not isinstance(metadata, dict):
        return payload

    # ``langgraph_sdk`` routes on the payload ``id``, which the protocol puts on
    # ``message-start`` alone. Every later frame falls back to a slot that only
    # resolves while exactly one message is open, so a graph with parallel model
    # calls loses them. ``message-start`` already uses this exact value, which
    # makes the stamp a reconstruction rather than an invention.
...
def unwrap_protocol_data(method: str, data: Any) -> Any:
    ...
    The metadata half is unwrapped away, so anything the consumer still needs
    from it is carried onto the payload first. See ``_carry_routing_fields``.
    """
    if method not in {"messages", "tools"}:
        return data
    if isinstance(data, tuple) and len(data) >= 1:
        return _carry_routing_fields(data[0], data[1] if len(data) > 1 else None)
    if (
...
def sanitize_event(event: ProtocolEvent) -> ProtocolEvent:
    ...
    # The spec carries the producing node on the envelope, and the JS
    # ``MessageAssembler`` keys its active-message map on namespace + node.
    # The metadata half is its only source, so lift it before unwrap drops it.
    if (
        method in {"messages", "tools"}
        and isinstance(data, (tuple, list))
        and len(data) > 1
        and isinstance(data[1], dict)
        and data[1].get("langgraph_node")
    ):
        params.setdefault("node", data[1]["langgraph_node"])
...
def encode_sse(event: ProtocolEvent) -> str:
    ...
    sse = f"{id_line}event: message\ndata: {json.dumps(event)}\n\n"
    # global sse_counter
    # if sse_counter > 0:
    logger.info(f"SSE ret: {sse}")
        # sse_counter -= 1
    return sse
...
class LocalThreadSession:
    ...
    def _ensure_root_lifecycle_terminal(
